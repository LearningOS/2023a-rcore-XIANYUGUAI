...
impl Inode {
    ...
    /// link name to inode id under this inode
    ...
    pub fn unlink(&self, name: &str) -> bool {
        ...
        if let Some(inode_id ) = self.read_disk_inode(op) {
            let (inode_block_id, inode_block_offset) = _fs.get_disk_inode_pos(inode_id);
            get_block_cache(inode_block_id as usize, Arc::clone(&self.block_device))
                .lock()
                .modify(inode_block_offset, |inode: &mut DiskInode| {
                    inode.nlink -= 1;
    ...
    /// remove name from current inode
